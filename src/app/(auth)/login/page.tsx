...
function GoogleLoginButton({ onLogin }: { onLogin: (user: { id: string; name: string; phone: string; role: 'petani' | 'koordinator' | 'admin' }) => void }) {
  const router = useRouter()
  const [loading, setLoading] = useState(false)

  const handleGoogle = async () => {
    setLoading(true)
    // Demo: simulasi Google OAuth login
    // Produksi: ganti dengan NextAuth signIn('google') atau redirect ke Google OAuth
    await new Promise(r => setTimeout(r, 1500))
    onLogin({ id: 'google-user-1', name: 'Budi Santoso', phone: '08123456789', role: 'petani' })
    router.replace('/dashboard')
  }
  ...
  return (
    <button
      ...
      disabled={loading}
      className="w-full py-3.5 bg-white border border-gray-200 rounded-xl flex items-center justify-center gap-3 font-semibold text-gray-700 shadow-sm hover:bg-gray-50 active:bg-gray-100 transition-colors min-h-[52px]"
    >
      {loading ? (
